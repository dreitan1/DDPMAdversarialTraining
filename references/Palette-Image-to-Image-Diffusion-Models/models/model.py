...
class Palette(BaseModel):
    ...
    def set_input(self, data):
        ...
        self.cond_image = self.set_device(data.get('cond_image')).to(self.device)
        self.gt_image = self.set_device(data.get('gt_image')).to(self.device)
        self.mask = self.set_device(data.get('mask')).to(self.device) if data.get('mask') is not None else None
                ...
                if self.opt['distributed']:
                    ...
                    if self.task in ['inpainting', 'uncropping']:
                        self.output, self.visuals = self.netG.restoration(
                            y_cond=self.cond_image.to(self.device), 
                            y_t=self.cond_image.to(self.device), 
                            y_0=self.gt_image.to(self.device), 
                            mask=self.mask.to(self.device) if self.mask is not None else None,
                            sample_num=self.sample_num
                        )
                        print("Conditioned Image:", self.cond_image)
                        print("Output Image:", self.output)
                    else:
                        self.output, self.visuals = self.netG.restoration(
                            self.cond_image.to(self.device), 
                            sample_num=self.sample_num
                        )
                self.iter += self.batch_size
